...
def get_available_years(table_name: str):
    ...
    years = hbsir.utils.Argham(table_availability, default_end=default_end).get_numbers()
    years = list(years)
    years.sort()
    return years
...
def test_load_table(table_name, year):
    hbsir.load_table(table_name, year, on_missing="create", recreate=True, save_created=False)
